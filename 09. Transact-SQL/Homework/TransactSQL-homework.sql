...
AS
BEGIN
    RETURN (@sum * (@yearlyInterestRate / 12) * @numOfMonthS)
END
GO

USE Bank
SELECT Balance, dbo.ufn_CalculateBonus(Balance , 0.1, 1) as Bonus
FROM Accounts
GO
...
AS
    ...
    DECLARE @bonus money;
    SELECT @bonus = dbo.ufn_CalculateBonus(@balance, @yearlyInterestRate, 1)

    SELECT p.FirstName, p.LastName, a.Balance, @bonus AS Bonus
    FROM Accounts a
    JOIN Persons p
...
BEGIN
    ...
    WHERE 1 = [Bank].dbo.RegExpLike(LOWER(Name), @pattern)
    RETURN
END
GO
...
DROP FUNCTION [Bank].dbo.RegExpLike;
DROP assembly SqlRegularExpressions; 
